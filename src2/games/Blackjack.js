...
export class Blackjack extends Game
{
    ...
    async stand(game_message, username, bet_amount)
    {
        ...
        // reveal dealer's hand and play out
        await game_message.edit({
            flags: MessageFlags.IsComponentsV2,
            components: [MessageTemplates.blackjackMessage(username, this.player_hand, this.dealer_hand, bet_amount, this.cards, player_value, dealer_value, false)]
        });
        ...
        if(dealer_value > 21 || player_value > dealer_value)
        {
            await game_message.edit({
                flags: MessageFlags.IsComponentsV2,
                components: [MessageTemplates.blackjackResultMessage(username, this.player_hand, this.dealer_hand, bet_amount, player_value, dealer_value, 'win')]
            });
            // player wins
            res = { result: 'win', payout: bet_amount, message: game_message, player_value: player_value, dealer_value: dealer_value, base_payout: bet_amount };
        }
        else if(player_value < dealer_value)
        {
            ...
            await game_message.edit({
                flags: MessageFlags.IsComponentsV2,
                components: [MessageTemplates.blackjackResultMessage(username, this.player_hand, this.dealer_hand, bet_amount, player_value, dealer_value, 'loss')]
            });
            res = { result: 'loss', payout: -bet_amount, message: game_message, player_value: player_value, dealer_value: dealer_value, base_payout: bet_amount };
        }
        else
        {
            // push
            await game_message.edit({
                flags: MessageFlags.IsComponentsV2,
                components: [MessageTemplates.blackjackResultMessage(username, this.player_hand, this.dealer_hand, bet_amount, player_value, dealer_value, 'push')]
            });
            res = { result: 'push', payout: 0, message: game_message, player_value: player_value, dealer_value: dealer_value, base_payout: 0 };
        }
    ...
    async play(user, message, bet_amount)
    {
        ...
        const game_message = await message.reply({
            flags: MessageFlags.IsComponentsV2,
            components: [MessageTemplates.blackjackMessage(username, this.player_hand, this.dealer_hand, bet_amount, this.cards, player_value, "?", true)]
        });
        ...
        if(player_value === 21)
        {
            ...
            if(dealer_value === 21)
            {
                ...
                result = { result: 'push', payout: 0, message: game_message, player_value: player_value, dealer_value: dealer_value, base_payout: 0 };
            }
            else
            {
                ...
                result = { result: 'win', payout: blackjack_payout, message: game_message, player_value: player_value, dealer_value: dealer_value, base_payout: blackjack_payout };
            }
            ...
            collector.on('collect', async (interaction) =>
            {
                ...
                if(hit_or_stand === 'hit')
                {
                    ...
                    if(player_value > 21)
                    {
                        ...
                        result = { result: 'loss', payout: -bet_amount, message: game_message, player_value: player_value, dealer_value: dealer_value, base_payout: bet_amount };
                        collector.stop();
                        resolve(result);
                    }
                    else if(player_value === 21)
                    {
                        result = await this.stand(game_message, username, bet_amount);
                        collector.stop();
                        resolve(result);
                    }
                    else
